...
from garage.sampler import LocalSampler, MultiSampler, \
    WorkerFactory
from garage.tf.envs import TfEnv


def test_multi_sampler():
    ...
    sampler1 = MultiSampler.from_worker_factory(workers, policy, env)
...
def test_update_envs_env_update():
    ...
    workers = WorkerFactory(seed=100,
                            ...
                            n_workers=n_workers)
    sampler = MultiSampler.from_worker_factory(workers, policy, env)
...
def test_init_with_env_updates():
    ...
    workers = WorkerFactory(seed=100,
                            max_path_length=max_path_length,
                            n_workers=n_workers)
    sampler = MultiSampler.from_worker_factory(workers,
                                               policy,
...
def test_obtain_exact_trajectories():
    ...
    workers = WorkerFactory(seed=100,
                            max_path_length=max_path_length,
                            n_workers=n_workers)
    sampler = MultiSampler.from_worker_factory(workers, policies, envs=env)
    n_traj_per_worker = 3
